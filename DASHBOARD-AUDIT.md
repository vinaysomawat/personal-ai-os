...
| 2 | Logo lockup | 16px/700, no icon box in current source | OPEN | Left unchanged — repo's icon-badge treatment predates this specific revision; a call to drop it wasn't clearly warranted by a source I can't fully trust here (see row 1's caveat class). |
...
| 9 | Ask Brain / module-advisor trigger | 12px×7px padding, 12.5px text | **FIXED** | `TopNav.tsx` trigger button now `px-3 py-[7px] text-[12.5px]`. |
...
| 36 | Goal Progress card padding | 16px/18px | **FIXED** | `px-4 py-3.5` → `py-4 px-[18px]`. |
| 37 | Goal Progress header letter-spacing | 0.4px | **FIXED** | `tracking-wide` → `tracking-[0.4px]`. |
| 38 | Goal row bar track radius | 4px | **FIXED** | `rounded-full` → `rounded-[4px]`. |
| — | (QuickStats↔Goal Progress internal gap) | 14px | **FIXED** | Found during implementation (not in original 74-row count): outer `space-y-3` → `space-y-3.5`. |
...
| 65 | Goals (cross-module) card | New card: badges, qualitative/quantitative rows, footer link | **FIXED** | Added in full. Data was already fetched by `getDashboardData()` (`crossModuleGoals`, previously only feeding Ask Brain's prompt context) — extended it with `achieved`/`current`/`target` raw fields (the pre-existing `progress` string kept untouched for the AI-prompt consumer) and rendered the card matching the source's badge/bar/qualitative-label recipe. |
...
| 72 | Quick Add floating button | 52px, 28px offset, 24px icon, plain shadow | **FIXED** | `w-12 h-12` (48px) → `w-[52px] h-[52px]`; `bottom-6 right-6` (24px) → `bottom-7 right-7` on desktop (kept lifted on mobile to clear the new bottom nav, see row 14); `Plus size={22}` → `24`; removed the extra `ring-4 ring-background` embellishment the source doesn't have. |
...
- **62 rows FIXED** (including one gap found during implementation, not in the original 74; rows 1 and 7 moved here after the truncation bug was resolved and their exact values confirmed).
- **10 rows MATCH** (already correct, confirmed, untouched; row 8 moved here for the same reason).
- **2 rows OPEN** (row 2, logo lockup — still genuinely source-ambiguous even after the truncation fix; row 73, Quick Add's flagged product-behavior question, with its dependent row 74 left N/A alongside it).
