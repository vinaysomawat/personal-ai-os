...
// The daily reading's structured output (astrology.md 3.9) — only ever
// produced for period 'daily' (monthly/yearly stay prose, since "avoid X
// today" and a Moon-transit mood forecast don't translate to a month/year
// scale the same way). `remediation` is populated by the app from the same
// curated getRemediation() table the standalone Remediation card uses, not
// generated by the model — the AI prompt only ever asks for summary/
// favorableFor/avoid/moodForecast.
...
// Choghadiya — 8 day blocks (sunrise-sunset) + 8 night blocks (sunset-next
// sunrise), each labeled from a fixed 7-name cycle whose starting point is
// weekday-dependent (astrology.md 3.9's "daily productivity layer"), a
// standard practical scheduling layer distinct from the 3 inauspicious
// windows above. See panchang.ts's getChoghadiya() for the actual tables.
export interface ChoghadiyaBlock {
  name: string
