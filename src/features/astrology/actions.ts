...
export async function getAstrologyReading(profile: AstrologyProfile, period: ReadingPeriod, lang: Lang = 'en'): Promise<string> {
  ...
  return askAI(
    'astrology_reading',
    prompt,
    `You are a traditional Vedic astrology reader. Given a natal chart summary (including Navamsa/D9), the current Vimshottari and Yogini dasha periods, current planetary transits, and gochara house placements, write a short, specific horoscope reading — exactly 4 crisp sentences, each under 15 words and each making one point (theme, work/money, relationships/health, advice), grounded in the actual positions given (name the relevant planet/house briefly), not generic. Frame it as traditional guidance, not certainty. Plain text only, no markdown formatting. Under 70 words.${LANGUAGE_INSTRUCTION[lang]}`,
...
export async function getStructuredDailyReading(profile: AstrologyProfile, lang: Lang = 'en'): Promise<DailyReading> {
  ...
  const raw = await askAI(
    'astrology_reading',
    prompt,
    `You are a traditional Vedic astrology reader. Given a natal chart summary (including Navamsa/D9), the current Vimshottari and Yogini dasha periods, current planetary transits, gochara house placements, the transiting Moon's relation to the natal Moon, and today's tithi, return ONLY a JSON object in this exact shape: {"summary": string, "favorableFor": string[], "avoid": string[], "moodForecast": string}. "summary" is ONE short sentence (max 20 words) naming the single most important influence today, grounded in the actual positions given, plain language, not generic. "favorableFor" is 2-3 crisp actionable phrases of 3-8 words each (e.g. "financial decisions", "talking to seniors") grounded in the dasha+gochara+panchang context. "avoid" is 1-2 crisp phrases of 3-8 words each (e.g. "signing new agreements") grounded in the same context — no explanations or planet names inside these phrases. "moodForecast" is ONE short sentence (max 15 words) on how mood/emotional tone may run today, grounded specifically in the transiting Moon's position relative to the natal Moon (mention Chandrashtama if flagged) — frame as traditional astrological interpretation, constructive and even-handed, never a clinical or diagnostic claim about actual emotional state. Frame everything as traditional guidance, not certainty. No markdown. Return ONLY the JSON object, nothing else.${LANGUAGE_INSTRUCTION[lang]}`,
...
export async function getAstrologyCharacteristics(profile: AstrologyProfile, lang: Lang = 'en'): Promise<{ text: string; generatedAt: string }> {
  ...
  return askAIWithMeta(
    ...
    context,
    `You are a traditional Vedic astrology reader. Given a natal chart summary (Lagna, planetary placements with houses/nakshatras, and Navamsa/D9), write a grounded characteristics/personality profile: Lagna qualities, Moon sign (mind/emotional nature), notable strong and weak planetary placements, and general tendencies. exactly 5 crisp sentences, each under 16 words and each making one point (Lagna nature, Moon/mind, key strength, key challenge, general tendency), grounded only in the actual chart data given — never invent a placement not in the data. Frame as traditional astrological interpretation, not a definitive personal judgment. Plain text only, no markdown. Under 90 words.${LANGUAGE_INSTRUCTION[lang]}`
  )
}
