...
async function handleCallbackQuery(moduleName: ModuleName, update: TelegramUpdate): Promise<void> {
  ...
  if (data.startsWith('task_done:')) {
    ...
    if (isUndoableTable(table) && id) {
      const db = createServiceClient()
      // food_log goes through the shared helper, which also subtracts the
      // entry back out of that day's health_metrics calories/protein total.
      let row: Record<string, unknown> | null
      if (table === 'food_log') {
        const { removeFoodLogEntry } = await import('@/features/health/food-log')
        row = await removeFoodLogEntry(db, id)
      } else {
