...
export async function execute(action: Record<string, unknown>, db: SupabaseClient, userId: string): Promise<ModuleReply> {
  switch (action.action) {
    ...
    case 'ask': {
      ...
      const answer = await askCodingMentor(String(action.question), { recentSolved, currentStreakDays: statsRes.currentStreak })
      return `🧑‍💻 *Coding Mentor:*\n\n${answer}`
    }
    case 'undo_last': {
      const lastQuestion = await db.from('coding_daily_questions').select('id, completed_at, question:coding_questions(title)').eq('user_id', userId).eq('completed', true).order('completed_at', { ascending: false }).limit(1).maybeSingle()
      const q = lastQuestion.data
      if (!q) return `❌ Nothing recent to undo.`
