...
**Update (advisor-panel re-sync pass):** the panel header/shell is no longer a dropdown — `AIAdvisorProvider` is now a full-height right-side drawer (`14px/700` title, `var(--card-pad-lg)`-padded header, plain `✕` close) per `Quiz-Modal-and-Advisor-Shell-AUDIT.md` Part 2, and this pass additionally fixed a header padding bug and the panel's entrance motion (see that file's S2/S7/S8) and gave Ask Brain its correct 440px width (it turned out to reuse this same shared component, not a separate shell). The tab-selector style is still the filled-pill segmented control, not design's underline tabs — re-confirmed as an intentional, unchanged OPEN judgment call (kept for consistency with every other 2/3/4-way tab toggle in the app, per Part 2's S5) rather than an oversight. See `audit/Ask-Brain-and-Executive-Summary-AUDIT.md` for the rest of Ask Brain's tabs and the Executive Summary panel (found to be a separate hand-rolled component in `PlannerView.tsx`, not sharing this shell at all).
