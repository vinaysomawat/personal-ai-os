...
| F9 | Legend | (2026-08-10) Colored square swatches (9×9px, 2px radius, plain `var(--good)`/`var(--warn)`/`var(--risk)`/`var(--border)`), `gap:16px` | **FIXED** | Design switched from emoji dots back to colored squares this round — updated `CodingCalendar.tsx`'s legend row from emoji spans to `9px` colored-square swatches, `gap-4` (16px, was `gap-3.5`). Copy for the 4th item kept as **"No assignment"** rather than design's literal "None" — a real semantic difference in this app (a gray day specifically means no question was assigned that day, e.g. a rest day, not merely "no activity logged") — OPEN, kept for accuracy. |
| F10 | Header row spacing | `margin-bottom:14px; gap:10px; flex-wrap:wrap` (was `margin-bottom:12px`, no wrap) | **FIXED** (2026-08-10) | Nav/month-label row: `mb-3`→`mb-3.5`, added `flex-wrap`. |
| F11 | Stats row (new, 2026-08-10) | 3-col grid, `gap:8px`, `margin-bottom:14px`, tiles: `bg-surface-2 rounded-[10px] padding:9px 12px`, value `16px/700`, label `10.5px` tertiary — "Current streak" / "Best streak" / "Active days" | **FIXED** | Was entirely missing. Added as a 3-tile grid between the calendar header and weekday row. "Current streak"/"Best streak" reuse the page's already-computed `codingStats.currentStreak`/`.longestStreak` (single source of truth, not a second calendar-scoped derivation). "Active days" is a new deterministic value computed in `CodingCalendar.tsx`: `% of past days (across the full fetched window) with status 'solved' or 'partial'` — same "count/total, rounded" recipe as `computeCodingStats`' `completionRate`, just over calendar days instead of assigned questions. No AI involved (Product Principle 2). |
...
| G2 | **"Today's Quiz" — NOT BUILT, flagged for a separate go-ahead** | New section (2026-08-10) replacing G1: a 10-question hardcoded DSA/algorithms warm-up quiz (`TODAYS_QUIZ_BANK` in the design's mock script — time complexity, data structures, Big-O, tree traversal, sorting, etc.), multiple-choice buttons, an explanation line per question, a `todaysQuizDone`/score badge in the header, and a "↻ Retake" button. | **ADDED — unresolved, not implemented** | This is a genuinely new, self-contained feature (question bank + answer state + scoring + retake), not a style/layout diff — matches this project's CLAUDE.md "give a spec first for non-trivial features" workflow rather than something to silently build during a design-fidelity pass. Deliberately not implemented. Needs a product decision: static hardcoded bank (matching the mock exactly) vs. AI-generated (`askAI`, matching the app's existing Learning/Career quiz patterns) vs. skipped entirely. |
...
All MISMATCH/MISSING rows across Sections A–K are **FIXED**. The Contribution Calendar gained the most real functionality beyond styling: today/selected-day ring indicators, future-day dashed treatment, click-to-select with a detail readout, and a month summary line were all **entirely missing** — the calendar was previously a static, non-interactive color grid; 2026-08-10 added a 3-tile streak/active-days stats row and switched the legend from emoji to colored squares (F10/F11). Every status/difficulty badge across the module (Today's Question, Daily Tech Read, Practice Log, Recommended for You) was converted from a filled colored pill to design's `bg-border` + colored-text convention. Page structure now matches design's grouping (Question/Read cards + Calendar side by side; Practice Log → Recommended for You in that order — Goals dropped from both the design and the product, see G1). Remaining OPEN items (deliberate, not oversights): the struggle-rate 60%/70% threshold (real pre-existing scoring logic), multi-question daily assignments, Practice Log's 8 filters + row-level interactive actions, the AI-gated "Get Recommendations" flow, the settings gear's icon-over-glyph choice, and the Difficulty Progression chart (removed from the product 2026-08-10, see `audit/CHANGELOG.md`). **Unresolved, needs a go-ahead:** the new "Today's Quiz" section (G2) — a real new feature, not a style diff, deliberately not built.
