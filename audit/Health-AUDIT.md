...
| C8 | Section label style + text | `11px/700/uppercase/0.4px-tracking`; labels **"Warmup" / "Exercises" / "Cardio Finisher" / "Cooldown" / "Coach Tips"** | **FIXED** | Style: was `text-xs uppercase tracking-wider` (no bold) → corrected across all 5 section labels. Text: design's "Warmup"/"Cooldown" are single words (no hyphen) — repo had "Warm-up"/"Cool-down" → corrected. |
