...
|---|---|---|---|---|
| 1 | Header container | `headerStyle`: `z-index: 20`, `padding: 14px 20px` | **FIXED**, then intentionally tightened further | `TopNav.tsx`: `z-30`→`z-20`, padding moved from an inner `px-4 md:px-5 h-12` row to header-level `px-5 py-3.5` (exact spec match). Vertical padding then hand-tightened again 2026-08-11 to `py-2` — a deliberate deviation past the literal design number, not a design-fidelity miss. |
...
| 13 | Profile dropdown items | Documents, Settings, Sign out; `profileMenuItemStyle`: 9px/10px padding, 13px text, 7px radius | **FIXED**, Documents entry later removed | Added a Documents entry above Settings in `ProfileMenu.tsx`; confirmed exact via the complete source and applied: `py-[9px] px-2.5 text-[13px] rounded-[7px]`. The Documents entry itself was removed 2026-08-11 along with the whole module (not a style regression — see DOCUMENTS-AUDIT.md); the padding/text/radius styling this row verified still applies to the remaining Settings/Sign out items. |
...
| 14 | Mobile nav | Fixed bottom bar (58px, 4 primary + More sheet); `z-index`: bar `45`, More-sheet overlay `46`, More-sheet panel `47` | **FIXED**, More sheet later lost a Documents item | Rebuilt in `TopNav.tsx`: fixed 58px bottom bar (Home/Planner/Health/Finance) + a More sheet (originally Career/Learning/Coding/Documents/Settings, now Career/Learning/Coding/Settings since Documents was removed 2026-08-11 — see DOCUMENTS-AUDIT.md). `layout.tsx`'s `<main>` gained `pb-[58px] md:pb-0` so content isn't hidden behind it; `QuickAdd`'s floating button was lifted to `bottom-[86px]` on mobile to clear it. Z-index trio (previously all `z-40`, undifferentiated) now split to `z-[45]`/`z-[46]`/`z-[47]` matching the latest design extraction's literal values. |
| 15 | Density system (`data-density`) | `[data-density]`/`[data-density="compact"]` CSS vars | **FIXED**, default later flipped | Added to `globals.css`; `<html data-density="comfortable">` originally set in `layout.tsx`. `Card.tsx`'s default padding reads `var(--card-pad-lg)` so every large card responds to it. 2026-08-11: `compact` made the app's actual fixed default (`data-density="compact"`) per direct request — still no in-app toggle (matches the design tool's own dev-only "Tweaks" panel, not a visible app feature), just a different fixed value. `Modal.tsx` also switched from a hardcoded `p-6` to `p-[var(--modal-pad)]` the same day, so modals now respond to this too (see MODAL-AUDIT.md). |

## Dashboard main content (`isDashboard` block, top to bottom)

| # | Element | Design spec | Verdict | Resolution |
|---|---|---|---|---|
| 16 | `<main>` wrapper | 1180px/28px-32px-0/20px gap | **FIXED**, then intentionally tightened further | `layout.tsx`: `px-8 pt-7 pb-[58px] md:pb-0` (32px/28px/0 desktop) matched the spec exactly. `DashboardView.tsx` root `space-y-4` → `space-y-5` (20px) also matched. 2026-08-11: both hand-tightened past the literal spec per direct "make it compact" request — `layout.tsx` now `px-6 pt-5`, and all 8 module pages' root wrapper (not just Dashboard) now `space-y-3` (12px), not just this one page. |
| 17 | Date/greeting line | 13px tertiary | MATCH | Unchanged. |
| 18 | "Dashboard" title | 34px/700/-0.05em | **FIXED** | `text-[34px] tracking-[-0.05em]` (was responsive 24-30px `tracking-tight`). |
...
| 24 | Hero grid gap | 20px | **FIXED**, then intentionally tightened further | `gap-4` → `gap-5` matched the spec exactly. 2026-08-11: tightened again to `gap-3` (12px) per direct "make it compact" request, along with every other major page-level grid gap app-wide. |
...
| 28 | Life Score number | -0.05em letter-spacing | **FIXED** | `tracking-tight` → `tracking-[-0.05em]`. |
...
| 32 | Quick Stat tile | 16px radius, 14px/16px padding via density var | **FIXED**, now stale since the default flipped | Radius/shadow/padding numbers matched at the time; `QuickStats.tsx` still hardcodes `px-4 py-3.5` rather than reading `var(--card-pad-md)`. That was harmless while `comfortable` (16px) was the default — now that `compact` (9px 10px) is the fixed default (row 15), this tile is the one card that didn't shrink with the rest of the app. Worth wiring to the density var in a follow-up pass, not done as part of 2026-08-11's changes. |
