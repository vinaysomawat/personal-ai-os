...
**Scope note — GoalsCard is explicitly OUT OF SCOPE.** It's a component (previously also shared with Learning and Coding; both removed their own GoalsCard instance from the UI since, so Career is now the only page still rendering it — see `audit/CHANGELOG.md`), and its dual-mode structure (progress bar for quantitative goals like coding streak, checkbox for qualitative ones) is real functionality no static mock attempts to represent. Changing it would ripple into the shared component, same "shared blast radius" category as the advisor-panel shell flagged (twice now) as its own separate audit. Not touched here.

**2026-08-10 re-sync note (round 2):** the design's own hand-rolled "Goals" mock inside `isCareer` (a `careerGoals` list with toggle/delete + a `newCareerGoalText` add-input, 19 lines) was **removed from the design entirely** this round — REMOVED relative to the prior sync. No code change needed: the repo never mirrored this exact markup in the first place (per the scope note above, it always rendered the real shared `GoalsCard` instead, treated as OPEN/kept). The design simply cleaned up its own now-redundant mock; the "shared architecture, out of scope" reasoning stands regardless.

**2026-08-10 re-sync note (round 4):** the design's card order changed from **Career Profile → Applications → Job Alerts → Interview Prep** to **Applications → Job Alerts → Interview Prep → Career Profile** (content byte-identical, confirmed via diff — a pure reorder, Career Profile moved to last). No code change needed: `CareerView.tsx` already renders `Applications` → `Job Alerts` → `Interview Prep — Topic Quiz` → `Career Profile` in that exact order (confirmed by line number). The design simply caught up to match what the repo already had.
