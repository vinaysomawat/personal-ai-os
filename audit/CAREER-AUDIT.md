...
**2026-08-10 re-sync note:** the design's own hand-rolled "Goals" mock inside `isCareer` (a `careerGoals` list with toggle/delete + a `newCareerGoalText` add-input, 19 lines) was **removed from the design entirely** this round — REMOVED relative to the prior sync. No code change needed: the repo never mirrored this exact markup in the first place (per the scope note above, it always rendered the real shared `GoalsCard` instead, treated as OPEN/kept). The design simply cleaned up its own now-redundant mock; the "shared architecture, out of scope" reasoning stands regardless.
